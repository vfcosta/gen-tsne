...
def calculate(paths, output_dir, enable_rmse=True, pca_components=None, frow=60, fcol=60, perplexity=30,
              n_iter=1000, save_data=True):
    df = build_grid(paths, pca_components=pca_components, frow=frow, fcol=fcol, perplexity=perplexity, n_iter=n_iter,
                    save_data=save_data, output_dir=output_dir)
    ...
    if enable_rmse:
        df_distances = calc_rmse(df, [32, 32, 3])
